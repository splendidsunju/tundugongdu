...
#include "leap_motion/CircleGesture.h"
        ...
        for (HandList::const_iterator current_hand = hand_list.begin(); current_hand != hand_list.end(); ++current_hand)
        {
            ...
            if (!frame.gestures().isEmpty())
            {
                ...
                for(Leap::GestureList::const_iterator gl = gestures.begin(); gl != gestures.end(); gl++)
                {
                    ...
                    {
                        std::vector<leap_motion::CircleGesture> circleGesture_msg_list; 
                        Leap::Gesture gesture = gestures[i];
                        leap_motion::CircleGesture ros_circle_gesture_msg;
                        ...
                        if(*current_hand == hands_for_gesture[i])
                        {
                            ...
                            if(gesture.type() == Leap::Gesture::TYPE_CIRCLE){
                                ...
                                if (circleGesture.pointable().direction().angleTo(circleGesture.normal()) <= Leap::PI/2) {
                                    ros_circle_gesture_msg.clockwiseness = "clockwise";
                                }
                                else {
                                    ros_circle_gesture_msg.clockwiseness = "counterclockwise";
                                }
                                    circleGesture_msg_list.push_back(ros_circle_gesture_msg);
                            ...
                            ros_gesture_msg.circleGesture_list =circleGesture_msg_list;
